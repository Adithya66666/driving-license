...
export default function PoliceLogin({ userid, usertype, username }) {
    ...
    const [formData, setFormData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
          const response = await fetch('http://localhost:5000/officerLogin', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                officerNumber: formData.officerNumber,
          ...
          if(response.status === 200){
            ...
            setFormData({officerNumber: '',password: ''})
            setLoading(false);
          }
    ...
    return <>
        <div className="w-screen h-screen flex">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="flex flex-col w-[400px]">
                        ...
                        <input
                            type="text"
                            name="officerNumber"
                            placeholder="Enter the police officer number"
            ...
            <div className="w-[50%] h-[100%] bg-black justify-center items-center flex">
                <img src="policelogin.jpg" alt="" className="h-[100%]"/>
            </div>
        </div>
    </>
}